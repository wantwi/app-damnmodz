...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['view'])) {
        ...
        if ($_GET['view'] === 'ongoingHome') {
                $selectQuery = "SELECT p.*, u.name FROM products p JOIN users u ON p.supplier_id = u.id WHERE p.status = 'ongoing' ORDER BY `p`.`dated` DESC LIMIT 50;";
    ...
    if(isset($_GET['query']) && !empty($_GET['query'])){
        ...
        if($_GET['status'] === 'all'){
            ...
            // Total rows for pagination
            $totalQuery = "SELECT COUNT(*) as total FROM products WHERE order_id LIKE :order_id";
            $totalStmt = $pdo->prepare($totalQuery);
        ...
        if ($_GET['status'] === 'new') {
                ...
                // Total rows for pagination
                $totalQuery = "SELECT COUNT(*) as total FROM products WHERE status = 'new' AND order_id LIKE :order_id";
                $totalStmt = $pdo->prepare($totalQuery);
                $totalStmt->bindValue(':order_id', $searchQuery);
                $totalStmt->execute();
                $totalRows = $totalStmt->fetch(PDO::FETCH_ASSOC)['total'];
    ...
    if (isset($_GET['ongoingchat'])) {
                $selectQuery = "SELECT p.*, c.date, u.name FROM products p JOIN chat c on p.hash_id = c.product_id JOIN users u ON p.supplier_id = u.id WHERE c.seen = 1 AND c.type = 'client' GROUP BY p.id ORDER BY `c`.`date` DESC;";
                $selectStmt = $pdo->prepare($selectQuery);
                $selectStmt->execute();
                $allOngoingOrders = $selectStmt->fetchAll(PDO::FETCH_ASSOC);
            if (!empty($allOngoingOrders)) {
    ...
    if (isset($_GET['ongoingAdminchat'])) {
                $selectQuery = "SELECT p.*, c.date, u.name FROM products p JOIN chat c on p.hash_id = c.product_id JOIN users u ON p.supplier_id = u.id WHERE c.seen = 1 AND c.type = 'client' AND p.supplier_id = 1 GROUP BY p.id ORDER BY `c`.`date` DESC;";
                $selectStmt = $pdo->prepare($selectQuery);
                $selectStmt->execute();
                $allOngoingOrders = $selectStmt->fetchAll(PDO::FETCH_ASSOC);
            if (!empty($allOngoingOrders)) {
