...
$webhook_secret = '/xgD88#xHs{H9m&jSP)_ !=n&5mg0;2mTP;):Cp`%QD9G^uTM]';
//$webhook_secret = 'QRdRWNYTu_sV@x==|wSxKW[z &kul/pnvy4^&O<N1GYzY3._=w';
...
// Verify the request if a secret is set
if ($webhook_secret && isset($headers['X-Wc-Webhook-Signature'])) {
    $signature = $headers['X-Wc-Webhook-Signature'];
    $computed_signature = base64_encode(hash_hmac('sha256', $request_body, $webhook_secret, true));

    if (!hash_equals($signature, $computed_signature)) {
        newLogs('woo', "Invalid webhook signature", 'error');
...
// Respond to WooCommerce to acknowledge receipt
